...
export default function StepNewPage() {
  ...
  const handleNext = (nextStep: number) => {
    router.push(`/add-church-new/${nextStep}`);
  };

  const handleBack = (prevStep: number) => {
    router.push(`/add-church-new/${prevStep}`);
  };
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "32px 24px 60px", position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div style={{ fontSize: "20px", fontWeight: 800, color: "var(--cn-ink)" }}>Add Your Church (3-Step Fast)</div>
          </div>
          <button className="btn-secondary" onClick={() => router.push('/add-church-new')}>
